...
Kakutou::Kakutou(IWorld * world, const Vector2 & position, const int& _number, const float _speed, int input_pad)
	: Actor2D(world, "player", position, std::make_shared<Box>(Vector2(-5, 75), Vector2(117, 149)), _number, _speed, input_pad)//Vector2(5, 76), Vector2(108, 118)
	, speed(_speed)
{
}
...
void Kakutou::OnUpdate(float deltaTime)
{
...
void Kakutou::OnDraw(Renderer & renderer)
{
...
// 二点間の平均値から中心座標を割り出す
Vector2 Kakutou::Average_Position() {
	Vector2 pos{ position.x + position.x + 112, position.y + 80 + position.y + 144 };
	return Vector2(pos.x / 2, pos.y / 2);
}

void Kakutou::OnFinalize()
{
}

void Kakutou::OnMessage(EventMessage message, void * param)
{
}

void Kakutou::OnCollide(const HitInfo & hitInfo)
{
